...
MainWindow::MainWindow(QWidget *parent)
  ...
  //         int decoded_samples = opus_decode(decoder, (const unsigned char*)datagram.data(), datagram.size(), decoded_data, frame_size, 0);
  //         if (decoded_samples > 0) {
  //             QByteArray data((const char*)decoded_data, decoded_samples * sizeof(opus_int16));
  //             audioOutput->start()->write(data);
  //     }
  // });
  ...
  //       int encoded_bytes = opus_encode(encoder, (const opus_int16*)data.data(), frame_size, encoded_data, 4000);
  //       if (encoded_bytes > 0) {
  //         voiceSocket->writeDatagram((const char*)encoded_data, encoded_bytes, QHostAddress::LocalHost, 8081);
  //       }
  //   });

}

MainWindow::~MainWindow() { /* opus_encoder_destroy(encoder); opus_decoder_destroy(decoder); */ delete chatSocket; /* delete voiceSocket; delete audioInput; delete audioOutput; delete format; */ delete ui; }
...
{
    if( ( ( event->key() == Qt::Key_Return ) && ui->stackedWidget->currentIndex() == 1 ) && event->key() != Qt::Key_Shift ) {
        chatSocket->write(
            "{"
            " \"Message\": \"" +
            ui->textEdit->toPlainText().toUtf8() +
